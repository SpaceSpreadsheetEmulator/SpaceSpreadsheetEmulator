...
public sealed class InspectorSettingsStore
{
    ...
    public InspectorSettingsStore(string configurationDirectory)
    {
    ...
    public InspectorSettings Load()
    {
        try
        {
            ...
            InspectorSettings? settings = JsonSerializer.Deserialize<InspectorSettings>(File.ReadAllText(settingsPath));
    ...
    public void Save(InspectorSettings settings)
    {
        ...
        File.WriteAllText(temporaryPath, JsonSerializer.Serialize(Normalize(settings), SerializerOptions));
