...
internal sealed class TestStaticDataArtifact : IAsyncDisposable
{
    ...
    private TestStaticDataArtifact(
    ...
    public static async Task<TestStaticDataArtifact> CreateAsync(IFileSystem fileSystem)
    {
        ArgumentNullException.ThrowIfNull(fileSystem);
        string root = fileSystem.Path.Combine(
            fileSystem.Path.GetTempPath(),
            $"sse-worker-{Guid.NewGuid():N}");
        ...
        string artifact = await new StaticDataPromoter(fileSystem, TimeProvider.System).PromoteAsync(
            ...
            3_396_210,
            hash);
        return new TestStaticDataArtifact(fileSystem, root, artifact);
    }
