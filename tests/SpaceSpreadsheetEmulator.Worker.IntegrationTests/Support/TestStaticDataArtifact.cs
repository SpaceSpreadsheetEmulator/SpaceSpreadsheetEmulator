...
internal sealed class TestStaticDataArtifact : IAsyncDisposable
{
    private TestStaticDataArtifact(string root, string artifactDirectory)
    {
        Root = root;
    ...
    public static async Task<TestStaticDataArtifact> CreateAsync()
    {
        string root = Path.Combine(Path.GetTempPath(), $"sse-worker-{Guid.NewGuid():N}");
        ...
        using (ZipArchive archive = ZipFile.Open(archivePath, ZipArchiveMode.Create))
        {
            ...
            Write(archive, "npcStations.jsonl", "{\"_key\":60000004,\"name\":{\"en\":\"New Caldari Prime\"},\"solarSystemID\":30002780,\"typeID\":1531,\"ownerID\":1000002}");
        ...
        string artifact = await StaticDataPromoter.PromoteAsync(
            ...
            3_396_210,
            hash);
        return new TestStaticDataArtifact(root, artifact);
    }
